...
const resultStyles = {
    valid: { box: 'bg-emerald-50 dark:bg-emerald-950/50 ring-emerald-200 dark:ring-emerald-900', badge: 'bg-emerald-600', label: 'Valid credential' },
    expired: { box: 'bg-orange-50 ring-orange-200 dark:ring-orange-900', badge: 'bg-orange-500', label: 'Expired credential' },
    revoked: { box: 'bg-rose-50 dark:bg-rose-950/50 ring-rose-200 dark:ring-rose-900', badge: 'bg-rose-600', label: 'Revoked credential' },
...
<template>
    <div class="bg-gradient-to-b from-brand-700 to-brand-900 px-4 py-16 text-center sm:py-24">
        ...
        <p class="mx-auto mt-3 max-w-xl text-brand-100">
            Enter the credential number (or scan the QR code on the document) to instantly confirm its authenticity.
        </p>
    ...
    <div class="mx-auto max-w-xl px-4 pb-16">
        ...
        >
            ...
            <p v-else class="mt-4 text-sm text-slate-600 dark:text-slate-400">
                No valid credential matches that number. Check for typos, or contact the issuer if you believe this is
                an error.
            </p>
        </div>
        ...
        <div v-if="!result" class="mt-12 grid grid-cols-1 gap-6 text-center sm:grid-cols-3">
            <div v-for="(item, i) in [
                { title: 'Find the number', text: 'It is printed on the credential, e.g. HSE-000123.' },
